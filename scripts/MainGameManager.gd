...
@export var quota_goal: int = 5
@export var time_limit_seconds: int = 300 
@export var info_request_penalty: int = 30 
...
## Path to the summary scene file updated to lowercase
...
# This array will be populated by the JSON loader
...
func load_applicants_from_json():
	...
	if error == OK:
		...
		if data_received is Dictionary and data_received.has("applicants"):
			...
			# Randomize the order of applicants every time the game starts
			applicants_data.shuffle()
		else:
			print("JSON Error: Unexpected format.")
	else:
...
func spawn_next_applicant():
	...
	# Create a temporary resource from the JSON data
...
func process_decision(approved: bool, data: ApplicantResource):
	current_processed += 1
	if data:
		if approved and data.is_unethical: ethical_score -= 10
		elif not approved and not data.is_unethical: ethical_score -= 5
		elif not approved and data.is_unethical: ethical_score += 5
		else: ethical_score += 2
...
func _on_request_button_pressed():
	...
	if current_document != null:
		if not current_document.evidence_revealed:
			...
			time_left = max(0, time_left - info_request_penalty)
			update_ui()
			request_button.disabled = true
			stop_button_highlight()
